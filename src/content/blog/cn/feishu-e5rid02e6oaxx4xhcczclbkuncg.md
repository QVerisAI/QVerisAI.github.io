...
description: 2.34% 写成 23.4%：金融 AI 最危险的幻觉，往往不是编出来的
pubDate: Jul 09 2026
...
![图片展示了一位身着深色西装的男子，坐在办公桌前，左手托腮沉思。他面前的电脑屏幕上显示着数据图表，旁边有代码、数据库等图标，还配有“%”标识。画面右侧有一个绿色的图表，同样带有“%”标识。背景是城市夜景，窗外高楼林立，其中东方明珠塔尤为显眼。此图与文档中提到的金融AI写报告时数据被模型错误写成23.4%的情景相契合，直观呈现了AI在金融数据分析与报告撰写中的场景。](../../../assets/blog-feishu-e5rid02e6oaxx4xhcczclbkuncg-1.png)
...
![该图片对应文档中介绍四类金融AI核验状态的内容，清晰展示了四种状态的释义：verified指核对一致，数据、公式、口径均可对应上，可正常输出；corrected表示发现错误并修正，比如2.34%被改回23.4%时会留下修正记录；uncertain是证据不足，因来源冲突、口径不清或数据不足，需提醒用户待确认；blocked则为阻断输出，针对关键数据缺失或风险过高的情况，宁可不输出也不做不确定的结论。这四类状态让用户能明确知晓内容的可信程度、修正情况、待确认项及高风险部分。](../../../assets/blog-feishu-e5rid02e6oaxx4xhcczclbkuncg-3.png)
...
![图片展示了QVeris Data Harness数据幻觉中工具JSON、LLM草稿和Claim Checker输出如何形成核验闭环的一个样例。样例中，工具JSON返回结构化数据，LLM草稿出现错误数字，Claim Checker给出修正结果，强调重点是让系统对账，而非让LLM心算。该图片与上下文紧密相关，直观呈现了QVeris解决AI自信有证据问题的核验过程，即工具给数据，模型写文字，QVeris负责对账。](../../../assets/blog-feishu-e5rid02e6oaxx4xhcczclbkuncg-4.png)
...
![图片展示了QVeris Data Harness给金融Agent补上的四件事。1. 接入真实能力，通过API等接入数据；2. 固化证据包，记录关键数据来源等；3. 核验关键输出，对账、修正自动完成；4. 治理与复盘，留下Trace Ledger，支持审计和人工确认。核心是让每一次输出都有证据链，而非让AI说得更像。该图与上下文紧密相关，直观呈现了QVeris Data Harness在金融Agent方面的功能优势。](../../../assets/blog-feishu-e5rid02e6oaxx4xhcczclbkuncg-5.png)
